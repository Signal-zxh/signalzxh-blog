...
- [ ] 首页支持按标签筛选（需确认/补充 API 或前端过滤方案）
- [ ] 管理后台增加「分类管理」页面（增删改，调用 `/api/categories`）
- [ ] 管理后台增加「标签管理」页面（增删改，调用 `/api/tags`）
- [ ] 后台编辑文章时支持修改分类与标签（目前创建已支持，编辑需确认）

### 测试

- [ ] 补 `handler/category_test.go`
- [ ] 补 `handler/tag_test.go`
- [ ] 补 `service/category_test.go`
- [ ] 补 `service/tag_test.go`
- [ ] 补 `db/category_test.go` / `db/tag_test.go`（sqlmock）

### 文档

- [ ] 更新 README：补充分类/标签 API 说明
